...
- 然后聊了聊项目,就项目问了我些问题
- 对于互联网的瓶颈你怎么看?
- 刚才你提到了很多技术,你的职业规划是什么样的?想走哪个方向?
- 平时是怎么学习的?
- 看过哪些书?

终面的问题其实技术性就不高了,主要还是聊了聊发展,职业规划这些.

### 4.10 阿里 后端 二面

- linux的io模型
- mysql与java交互也有一个类似tcp的交互过程,你能描述一下吗?
- http协议的请求头有哪些内容
- ConcurrentHashMap了解吗?与HashMap相比有哪些特性?怎么实现的?
- 一个线程怎么样算是安全的?
- 如何查看java虚拟机中运行线程的状态
- 你有什么问题吗?

### 快手一面+二面（记不全了，有印象的写在一起）

- 面形对象特性多态在java中的表现
- java泛型类型擦出
- 什么是运行时异常
- hashmap中如何处理哈希冲突，jdk1.8后做了什么优化（尾插，红黑树）
- concurrentHashmap实现，如何保证线程安全，jdk1.8后做了什么优化？
- AVL与红黑树有什么区别，为什么使用红黑树而不是用AVL？
- JVM的GC策略和GC算法有哪些
- TCP拥塞控制过程
- TCP中的clock_wait和time_wait有什么意义（就记得四次挥手中的那个）
- java线程池有哪些，什么特点
- 线程并发中sleep()和wait()有什么区别，分别是什么基类方法，是否会释放锁
- AotmicInteger的底层原理，死循环请求CAS如果一直循环会做什么处理
- Mysql事务隔离级别，可重复读的特点
- Mysql索引类型，B+树索引，B+树索引叶子结点一定要双链表串联嘛
- 算法：两个不同长度的链表，判断是否有公共节点，有的话输出公共节点，时间空间最优。
- 算法：一个有序数组，其中有一个元素有重复，返回该元素的出现次数，时间空间最优。（第一次出现最后一次出现分别做一次二分）
- 算法：P90问题。给出一个无序数组，找出如下元素，如果将数组排序，那么这个元素在第90%的位置，时间空间最优。（建堆遍历维护堆，O(nlogm),m为堆的大小）
